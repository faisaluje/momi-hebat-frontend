...
import { closeTransaksiBarangDialog } from '../store/actions';
import TransaksiBarangForm from './TransaksiBarangForm';

function TransaksiBarangDialog() {
  const dispatch = useDispatch();
  const { isLoading, props } = useSelector(({ barang }) => barang.transaksi.form);

  const handleClose = () => {
    dispatch(closeTransaksiBarangDialog());
  };
...
export default TransaksiBarangDialog;
