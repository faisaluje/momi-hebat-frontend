...
import { closePilihanPaketDialog } from './store/actions';
import PilihanPaketForm from './PilihanPaketForm';

function PilihanPaketDialog() {
  const dispatch = useDispatch();
  const { isLoading, props, data } = useSelector(({ pilihanPaket }) => pilihanPaket.form);

  const handleClose = () => {
    dispatch(closePilihanPaketDialog());
  };
...
export default PilihanPaketDialog;
